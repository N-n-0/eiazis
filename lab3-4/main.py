...
UPLOAD_FOLDER = "C:/Users/nikit/Desktop/bsuir/ЕЯзИИС/lab3/files"
SAVE_FOLDER = "C:/Users/nikit/Desktop/bsuir/ЕЯзИИС/lab3/saves"

app = Flask(__name__)
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['SAVE_FOLDER'] = SAVE_FOLDER

rename_synt_dict = {'ROOT': 'Сказуемое', 'pobj':'Предложное дополнение', 'nsubj':'Подлежащее',
                    'dobj':'Прямое дополнение', 'det':'Определитель'}
...
def text(id):
    ...
    sentences = make_sentences(id)
    return render_template("text.html", texts=texts, text=selected_text, id=selected_text[0], sentences=sentences)


@app.route('/sentence/<int:textId>/<int:sentenceId>')
def parsing(textId, sentenceId):
    sentences = make_sentences(textId)
    create_svg(sentences[sentenceId])

    # Отправка файла SVG в качестве ответа
    #return send_file(svg_path, mimetype='image/svg+xml')
    #return render_template('sentence.html', svg_path=svg_path)
    image_url = url_for('get_image')
    # Отображение шаблона с передачей URL-адреса изображения
    return render_template('sentence.html', image_url=image_url, id=textId)


@app.route('/image')
def get_image():
    # Здесь вы можете сгенерировать или получить путь к вашему изображению
    image_path = 'C:/Users/nikit/Desktop/bsuir/ЕЯзИИС/lab3/images/sentence.svg'
...
def word(text_id, id):
    ...
    words = select_all_words(text_id)
    definition = get_word_definition(value[0][0])
    synonyms, antonyms = get_synonyms_antonyms(value[0][0])
    return render_template("word.html", words=words, value=value, morphem=morphem_dict, size=12,
                           val_size=len(selected_word), id=id, texts=words, text_id=text_id,
                           definition=definition, synonyms=synonyms, antonyms=antonyms)


def make_sentences(textId):
    selected_text = select_text(textId)

    punctuations = r'[.!?]'
    sentences = re.split(punctuations, selected_text[5])

    sentences = [sentence.strip() for sentence in sentences if sentence.strip() and len(sentence)>=5]
    result = {}
...
def create_svg(sentence):
    nlp = spacy.load('ru_core_news_sm')

    doc = nlp(sentence)

    svg = displacy.render(doc, style='dep', jupyter=False)
    filename = 'sentence.svg'
    output_path = Path('C:/Users/nikit/Desktop/bsuir/ЕЯзИИС/lab3/images/' + filename)
